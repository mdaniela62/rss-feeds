### Comune di veneto ###
### Sostituisci "veneto" con il nome del Comune ###
...
# 🔧 CONFIGURAZIONE
COMUNE = "veneto"
BASE_URL = f"https://www.regione.{COMUNE}.it"
FEED_FILE = f"feeds/{COMUNE}.xml"
SOURCE_URL = f"{BASE_URL}/web/guest"
...
async def fetch_news():
    async with async_playwright() as p:
        ...
        blocks = await page.query_selector_all("div.w3-col.m4.s12")
        print(f"🔢 Trovati {len(blocks)} blocchi")
        news_items = []

        for block in blocks[:10]:
            title_el = await block.query_selector("h2")
            ...
            if title.lower() in ["avvisi", "notizie", "comunicati","senza titolo"]:
                continue
